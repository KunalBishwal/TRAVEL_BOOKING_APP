package travel.management.system;

import java.awt.*;
import java.awt.event.*;
import javax.swing.*;
import java.sql.*;

public class BookPackage extends JFrame {
    private JPanel contentPane;
    JTextField t1;
    Choice c1;
    String username;

    public BookPackage(String username) {
        this.username = username;
        setBounds(420, 220, 1100, 450);
        contentPane = new JPanel();
        setContentPane(contentPane);
        contentPane.setLayout(null);

        // Adding the image
        ImageIcon i1  = new ImageIcon(getClass().getResource("/resources/bookpackage.jpg"));
        Image i3 = i1.getImage().getScaledInstance(500, 300, Image.SCALE_DEFAULT);
        ImageIcon i2 = new ImageIcon(i3);
        JLabel la1 = new JLabel(i2);
        la1.setBounds(450, 50, 700, 300);
        add(la1);

        JLabel lblName = new JLabel("BOOK PACKAGE");
        lblName.setFont(new Font("Yu Mincho", Font.PLAIN, 20));
        lblName.setBounds(118, 11, 300, 53);
        contentPane.add(lblName);

        JLabel la2 = new JLabel("Username :");
        la2.setBounds(35, 70, 200, 14);
        contentPane.add(la2);

        JLabel l1 = new JLabel(username);
        l1.setBounds(271, 70, 200, 14);
        contentPane.add(l1);

        JLabel lblId = new JLabel("Select Package :");
        lblId.setBounds(35, 110, 200, 14);
        contentPane.add(lblId);

        c1 = new Choice();
        c1.add("Gold Package");
        c1.add("Silver Package");
        c1.add("Bronze Package");
        c1.setBounds(271, 110, 150, 30);
        contentPane.add(c1);

        JLabel la3 = new JLabel("Total Persons");
        la3.setBounds(35, 150, 200, 14);
        contentPane.add(la3);

        t1 = new JTextField("1"); // Default to 1 person
        t1.setBounds(271, 150, 150, 20);
        contentPane.add(t1);
        t1.setColumns(10);

        JLabel lbl1 = new JLabel("ID :");
        lbl1.setBounds(35, 190, 200, 14);
        contentPane.add(lbl1);

        JLabel l2 = new JLabel();
        l2.setBounds(271, 190, 200, 14);
        contentPane.add(l2);

        JLabel lbl2 = new JLabel("Number :");
        lbl2.setBounds(35, 230, 200, 14);
        contentPane.add(lbl2);

        JLabel l3 = new JLabel();
        l3.setBounds(271, 230, 200, 14);
        contentPane.add(l3);

        JLabel lbl3 = new JLabel("Phone :");
        lbl3.setBounds(35, 270, 200, 14);
        contentPane.add(lbl3);

        JLabel l4 = new JLabel();
        l4.setBounds(271, 270, 200, 14);
        contentPane.add(l4);

        JLabel lblDeposite = new JLabel("Total Price :");
        lblDeposite.setBounds(35, 310, 200, 14);
        contentPane.add(lblDeposite);

        JLabel l5 = new JLabel();
        l5.setBounds(271, 310, 200, 14);
        l5.setForeground(Color.RED);
        contentPane.add(l5);

        // Fetching customer details from the database
        try {
            Conn c = new Conn();
            ResultSet rs = c.s.executeQuery("select * from customer where username = '" + username + "'");
            if (rs.next()) {
                // Ensure column names match your database schema
                l2.setText(rs.getString("id"));
                l3.setText(rs.getString("number"));
                l4.setText(rs.getString("phone"));
            } else {
                JOptionPane.showMessageDialog(null, "Customer details not found for username: " + username);
            }
            rs.close();
        } catch (SQLException e) {
            e.printStackTrace();
        }

        // Check Price Button
        JButton b1 = new JButton("Check Price");
        b1.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String p = c1.getSelectedItem();
                int cost = 0;

                // Corrected logic: Use else-if to prevent multiple assignments
                if (p.equals("Gold Package")) {
                    cost = 12000;
                } else if (p.equals("Silver Package")) {
                    cost = 25000;
                } else if (p.equals("Bronze Package")) {
                    cost = 32000;
                }

                try {
                    int persons = Integer.parseInt(t1.getText());
                    cost *= persons;
                    l5.setText("Rs " + cost);
                } catch (NumberFormatException ex) {
                    JOptionPane.showMessageDialog(null, "Please enter a valid number of persons");
                }
            }
        });
        b1.setBounds(50, 350, 120, 30);
        b1.setBackground(Color.BLACK);
        b1.setForeground(Color.WHITE);
        contentPane.add(b1);

        // Book Button
        JButton btnBook = new JButton("Book");
        btnBook.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                Conn c = new Conn();

                String packageName = c1.getSelectedItem();
                String persons = t1.getText();
                String id = l2.getText();
                String number = l3.getText();
                String phone = l4.getText();
                String totalPrice = l5.getText();

                // Input validation
                if (totalPrice.equals("")) {
                    JOptionPane.showMessageDialog(null, "Please calculate the price first");
                    return;
                }

                String query = "insert into bookPackage values('" + username + "', '" + packageName + "', '" + persons + "', '" + id + "', '" + number + "', '" + phone + "', '" + totalPrice + "')";
                try {
                    c.s.executeUpdate(query);
                    JOptionPane.showMessageDialog(null, "Package Booked Successfully");
                    setVisible(false);
                } catch (Exception ee) {
                    ee.printStackTrace();
                    JOptionPane.showMessageDialog(null, "Error: " + ee.getMessage());
                }
            }
        });
        btnBook.setBounds(200, 350, 120, 30);
        btnBook.setBackground(Color.BLACK);
        btnBook.setForeground(Color.WHITE);
        contentPane.add(btnBook);

        // Back Button
        JButton btnBack = new JButton("Back");
        btnBack.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                setVisible(false);
            }
        });
        btnBack.setBounds(350, 350, 120, 30);
        btnBack.setBackground(Color.BLACK);
        btnBack.setForeground(Color.WHITE);
        contentPane.add(btnBack);

        getContentPane().setBackground(Color.WHITE);
    }

    public static void main(String[] args) {
        new BookPackage("Guest").setVisible(true);
    }
}
